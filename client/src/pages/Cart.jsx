...
const Cart = () => {
  ...
  const calculateTotals = () => {
    const subtotal = cartItems.reduce((total, item) => {
      const offerprice = parseFloat(item.productId.offerprice.replace(/[^0-9.-]+/g, "")) || 0;
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col lg:flex-row lg:space-x-6">
        <div className="flex-1 bg-white rounded-lg shadow-lg overflow-hidden">
          <h2 className="text-xl font-semibold border-b px-4 py-3 bg-gray-100">Cart Items</h2>
          {cartItems.length > 0 ? (
            <ul className="divide-y divide-gray-300">
              {cartItems.map((item) => (
                <li key={item.productId._id} className="flex items-center p-4 space-x-4 hover:bg-gray-50 transition">
                  <img
                  ...
                  <div className="flex-1 flex flex-col">
                    <h3 className="text-lg font-semibold">
                      <a href={item.productId.href} className="text-blue-600 hover:underline">
                        {item.productId.name}
                      </a>
                    </h3>
                    <div className="mt-2 flex items-center space-x-4">
                      <span className="text-gray-600 line-through">{item.productId.originalprice}</span>
                      <span className="text-xl font-semibold text-gray-900">{item.productId.offerprice}</span>
                      <span className="text-green-600">{item.productId.discount}</span>
                    </div>
        ...
        {cartItems.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg mt-6 lg:mt-0 lg:w-1/3">
            <h2 className="text-xl font-semibold border-b px-4 py-3 bg-gray-100">Order Summary</h2>
